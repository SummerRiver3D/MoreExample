...
from model import StableDiffusionRevisedInpaintPipeline
from model import UNet2DConditionRevisedModel as UNet2DConditionModel
from model import AsymmetricRevisedAutoencoderKL as AsymmetricAutoencoderKL
...
def retrieve_latents(
    encoder_output: torch.Tensor, generator: torch.Generator = None, sample_mode: str = "sample"
):
...
def log_validation(
    ...
    pipeline = StableDiffusionRevisedInpaintPipeline.from_pretrained(
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a SD-Inpaint training script.")
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--validation_image",
        type=str,
        default=["./src/test_image.jpg"], # None,#["examples/brushnet/src/test_image.jpg"],
        nargs="+",
        help=(
    ...
    parser.add_argument(
        "--validation_mask",
        type=str,
        default=["./src/test_mask.jpg"], #None,#["examples/brushnet/src/test_mask.jpg"],
        nargs="+",
        help=(
            "A set of paths to the paintingnet conditioning image be evaluated every `--validation_steps`"
            " and logged to `--report_to`. Provide either a matching number of `--validation_prompt`s, a"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Training SD-Inpaint with random mask"
        ),
    )
    parser.add_argument(
        "--ues_dual_encoders",
        ...
        default=False,
        help=(
            "Training SD-Inpaint with dual encoders"
        ),
    )
    parser.add_argument(
        "--use_half_zero",
        ...
        default=False,
        help=(
            "Training SD-Inpaint with zero-padded init"
        ),
    )
...
class MyWebDataset():
    ...
    def __call__(self,examples):
        ...
        for example in examples:
            ...
            mask_type_choice = random.random()

            if mask_type_choice < 0.45:
            ...
            elif mask_type_choice < 0.9:
                mask = self.random_mask_gen(image.shape[0],image.shape[1])[0][:,:,np.newaxis]
            else:
                p = random.uniform(0.05,0.20)            
                mask = np.random.choice([0, 1], size=(image.shape[0],image.shape[1]), p=[p, 1-p]).astype(np.bool_)[:,:,np.newaxis]
            
            if random.random() < 0.2 and mask_type_choice < 0.90:
            ...
            if random.random() < 0.1 and mask_type_choice < 0.90: # add pixel point
                p = random.uniform(0.01,0.05)            
                mask2 = np.random.choice([0, 1], size=(image.shape[0],image.shape[1]), p=[p, 1-p]).astype(np.bool_)[:,:,np.newaxis]
                mask = mask * mask2
            ...
            masked_image = (masked_image.astype(np.float32) / 127.5) - 1.0 # [-1,1]

            # mask = np.repeat(mask, 3, axis=-1)
            # mask = (mask.astype(np.float32) * 2.00000) - 1.00000 # [-1,1]
        ...
        if len(pixel_values) != 0:
            ...
            pixel_values, conditioning_pixel_values, masks, input_ids = torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([])
...
def main(args):
    ...
    if args.ues_dual_encoders:
        vae = AsymmetricAutoencoderKL.from_pretrained("/NAS/zhengdezhi/paper2/experiment_data/AsymmetricAutoencoderKL_pix_full", revision=args.revision, variant=args.variant)
    else:
    ...
    if args.ues_dual_encoders:
        unet.revise_inchannels(addition_channels=512, half_zero=args.use_half_zero)
    ...
    unet.cond_projection.train()
    unet.cond_projection.requires_grad_(True)
    unet.conv_in.train()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                condition_kwargs = {}
                if isinstance(vae, AsymmetricAutoencoderKL):
                    init_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)
                    init_image_condition = init_image.clone()
                    #init_image = _encode_vae_image(vae, init_image, None)
                    mask_condition = batch["masks"].to(dtype=weight_dtype)
                    condition_kwargs = {"image": init_image_condition, "mask": mask_condition}
                    conditioning_latents2 = vae.decode(conditioning_latents / vae.config.scaling_factor, return_dict=False, return_latent_state=True,**condition_kwargs)[0]
                    # conditioning_latents2 = conditioning_latents2 * vae.config.scaling_factor

                
                # # Debug
                # image = vae.decode(conditioning_latents / vae.config.scaling_factor, return_dict=False,**condition_kwargs)[0]
                # from diffusers.image_processor import VaeImageProcessor
                # from diffusers.pipelines.stable_diffusion.pipeline_output import StableDiffusionPipelineOutput
                # image_processor = VaeImageProcessor(vae_scale_factor=8, do_convert_rgb=True)
                # image = image_processor.postprocess(image, output_type='pil', do_denormalize=[True] * image.shape[0])
                # d = StableDiffusionPipelineOutput(images=image, nsfw_content_detected=None)[0]
                # for i in range(12):
                #     (d[i]).save(f"{i}.png")
                # exit()
                ...
                if isinstance(vae, AsymmetricAutoencoderKL):
                    latent_model_input = torch.cat([noisy_latents, masks, conditioning_latents, conditioning_latents2], dim=1)
                else:
                    latent_model_input = torch.cat([noisy_latents, masks, conditioning_latents], dim=1)
                # Predict the noise residual
                samples = unet(latent_model_input, timesteps, encoder_hidden_states)
                model_pred = samples.sample
                proj = samples.sample2
                ...
                # model_pred[batch["mask_types"] == 1] = model_pred[batch["mask_types"] == 1] * masks[batch["mask_types"] == 1]
                # target[batch["mask_types"] == 1] = target[batch["mask_types"] == 1] * masks[batch["mask_types"] == 1]

                if isinstance(vae, AsymmetricAutoencoderKL):
                    loss = F.mse_loss(model_pred, target, reduction="mean")# \
                    #    + 0.000001 * F.kl_div(F.log_softmax(proj, dim=1), F.softmax(latents, dim=1), reduction="batchmean")
                else:
                    loss = F.mse_loss(model_pred, target, reduction="mean")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    cur_save_path = os.path.join(args.output_dir, "checkpoint-last")
                    if not os.path.exists(cur_save_path):
                        accelerator.save_state(cur_save_path)
                        logger.info(f"Saved state to {cur_save_path}")
